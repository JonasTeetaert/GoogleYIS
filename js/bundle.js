...
/*#__PURE__*/
function () {
  function Layer(layer, cursor, data, popup) {
  ...
  _createClass(Layer, [{
      ...
      if (mq.matches) {
        this.paths.forEach(function (p) {
          p.addEventListener('mouseenter', function (e) {
            _this.cursor.classList.add('show');
          });
          p.addEventListener('mouseleave', function (e) {
            _this.cursor.classList.remove('show');
          });
          p.addEventListener('click', _this.openPopup.bind(_this));
        });
      } else {
        this.buttons.forEach(function (btn) {
          btn.addEventListener('click', _this.openPopup.bind(_this));
        });
      }
    }
  }, {
    ...
    value: function openPopup(e) {
      ...
      var data = this.data.filter(function (person) {
        return person.id === _this2.id;
      });
...
/*#__PURE__*/
function () {
  function Popup(node, coverLayer) {
...
/*#__PURE__*/
function () {
  function Smooth(popup, layerObjects) {
    ...
    this.popup = popup;
    this.layerObjects = layerObjects; // mouse
  ...
  _createClass(Smooth, [{
    ...
    value: function bindMethods() {
      ...
      ['run', 'requestAnimationFrame', 'cancelAnimationFrame', 'onPan'].forEach(function (fn) {
    ...
    value: function handleMouseMove(event) {
      ...
      if (event && !this.popup.status) {
        ...
        this.dom.cursor.style.transform = "translate3d(".concat(this.mouse.x, "px, ").concat(this.mouse.y, "px, 0)");
        this.dom.eyes.forEach(function (eye, i) {
          eye.setAttribute('transform', "translate(".concat(_this2.mouse.x, ", ").concat(_this2.mouse.y, ") scale(").concat(1 - i * 0.08, ")"));
        });
    ...
    value: function run() {
      ...
      var diffX = this.data.delta.x;
      var accX = diffX / this.config.width;
      var veloX = +accX;
      var skewX = veloX * 117.5;
      var diffY = this.data.delta.y;
      var accY = diffY / this.config.height;
      var veloY = +accY;
      var skewY = veloY * 1000; // console.log(skewX, skewY);
    ...
    value: function onPan(e) {
      ...
      this.layerObjects.forEach(function (layer) {
        layer.buttons.forEach(function (btn) {
          var pos = btn.getBoundingClientRect();
          var dist = Math.sqrt(Math.pow(Math.abs(_this3.center.x - pos.left), 2) + Math.pow(Math.abs(_this3.center.y - pos.top), 2)); // const dist = Math.hypot(center.x - pos.left, center.y - center.top);
    ...
    value: function moveParent() {
      ...
      if (mq.matches) {
        ...
        if (this.mouse.x < padding && border.left < -offset) {
          // left
          this.data.last.x += speed;
          this.data.current.x = this.data.last.x;
          this.dom.cover.style.transform = "translate3d(".concat(this.data.last.x, "px, ").concat(this.data.last.y, "px, 0)");
        } else if (this.mouse.x > window.innerWidth - padding && border.right > window.innerWidth + offset) {
          // right
          this.data.last.x -= speed;
          this.data.current.x = this.data.last.x;
          this.dom.cover.style.transform = "translate3d(".concat(this.data.last.x, "px, ").concat(this.data.last.y, "px, 0)");
        } else if (this.mouse.y < padding && border.top < -offset) {
          // top
          this.data.last.y += speed;
          this.data.current.y = this.data.last.y;
          this.dom.cover.style.transform = "translate3d(".concat(this.data.last.x, "px, ").concat(this.data.last.y, "px, 0)");
        } else if (this.mouse.y > window.innerHeight - padding && border.bottom > window.innerHeight + offset) {
          // bottom
          this.data.last.y -= speed;
          this.data.current.y = this.data.last.y;
          this.dom.cover.style.transform = "translate3d(".concat(this.data.last.x, "px, ").concat(this.data.last.y, "px, 0)");
        } else {// this.data.last.x = this.math.lerp(this.data.last.x, this.data.current.x, this.data.ease);
    ...
    key: "addMobileEvents",
    value: function addMobileEvents() {
      this.mc = new _hammerjs["default"].Manager(document.body);
      this.mc.add(new _hammerjs["default"].Pan({
        threshold: 0,
        pointers: 0
      }));
    ...
    value: function removeEvents() {
      this.dom.coverLayer.removeEventListener('mouseenter', this.requestAnimationFrame, {
        passive: true
      });
      this.dom.coverLayer.removeEventListener('mouseleave', this.cancelAnimationFrame, {
        passive: true
      });
    }
  }, {
    key: "init",
    value: function init() {// this.on();
    }
  }]);
...
},{"hammerjs":9}],4:[function(require,module,exports){
"use strict";
...
var _dataNl = _interopRequireDefault(require("./data-nl"));

var _dataFr = _interopRequireDefault(require("./data-fr"));
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var coverLayer = document.querySelector('.cover-layer');
  var smoothLayer = document.querySelector('.cover-layer .smooth-nav');
  var coverSvgLayer = document.querySelector('.cover-svg-layer');
  var skew = document.querySelector('.skew');
  var topLayer = document.querySelector('.top-layer');
  var holeLayer = document.querySelector('.hole-layer'); //const btnStart = document.querySelector('.top-layer button');

  var btnStart = document.querySelector('.eye__text');
  ...
  var layerObjects = [];
  var smooth;
  var borders = smoothLayer.getBoundingClientRect(); // functions

  var init = function init() {
    // create popup
    popup = new _Popup["default"](document.querySelector('.popup'), smoothLayer);
    handleMarkers();
  ...
  var setInitialButtonPos = function setInitialButtonPos() {
    var mq = window.matchMedia('(min-width: 801px)');
    document.querySelectorAll('.eye').forEach(function (eye, i) {
      eye.setAttribute('transform', "translate(".concat(window.innerWidth / 2, ", ").concat(window.innerHeight / 2 + 50, ")"));
    });
  };
  ...
  var start = function start(e) {
    ...
    smooth = new _Smooth["default"](popup, layerObjects);
    popup.setSmooth(smooth);
    var mq = window.matchMedia('(min-width: 801px)');

    if (mq.matches) {
      cursor.style.transform = "translate3d(".concat(e.clientX, "px, ").concat(e.clientY, "px, 0)");
      smooth.on();
      (0, _animejs["default"])({
        targets: '#hole .eye',
        width: 350,
  ...
  var handleMarkers = function handleMarkers() {
    var layers = coverSvgLayer.querySelectorAll('.layer');
    var lang = document.getElementsByTagName('html')[0].getAttribute('lang');

    if (lang == "nl") {
      layers.forEach(function (layer) {
        layerObjects.push(new _Layer["default"](layer, cursor, _dataNl["default"], popup));
      });
    }

    if (lang == "fr") {
      layers.forEach(function (layer) {
        layerObjects.push(new _Layer["default"](layer, cursor, _dataFr["default"], popup));
      });
    }
  };

  var initMobileControls = function initMobileControls() {
    // mobile controls
    var last_x = 0;
    var last_y = 0;
    var mc = new _hammerjs["default"].Manager(body);
    mc.add(new _hammerjs["default"].Pan({
      threshold: 0,
      pointers: 0
    }));
    ...
    function onPan(e) {
      if (e.type === 'panmove') {
        ...
        var value = "translate3d(".concat(pos.x, "px, ").concat(pos.y, "px, 0px)");
        smoothLayer.style.webkitTransform = value;
        smoothLayer.style.mozTransform = value;
        smoothLayer.style.transform = value;
      }

      layerObjects.forEach(function (layer) {
        layer.buttons.forEach(function (btn) {
          var pos = btn.getBoundingClientRect();
          var dist = Math.sqrt(Math.pow(Math.abs(center.x - pos.left), 2) + Math.pow(Math.abs(center.y - pos.top), 2)); // const dist = Math.hypot(center.x - pos.left, center.y - center.top);
...
},{"./Layer":1,"./Popup":2,"./Smooth":3,"./data-fr":4,"./data-nl":5,"animejs":8,"hammerjs":9,"nodelist-foreach-polyfill":10}],8:[function(require,module,exports){
/*
 * anime.js v3.1.0
